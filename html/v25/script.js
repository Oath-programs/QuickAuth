...
function saveOptions() {
	...
	options.window_layout = parseInt($("input[name=window_layout]:checked").val(), 10);
	...
	if ($("#keysecret").val()) {
		...
		options.secret = $("#keysecret").val().replace(/\W/g, '');
	}
...
$("document").ready(function() {
	...
	if (watch_version >= 2 && watch_version <= 3) {
		document.getElementById('pebble-stylesheet').setAttribute('href', 'pebble-'+watch_version+'.css');
	} else if (watch_version > 3) {
		document.getElementById('pebble-stylesheet').setAttribute('href', 'pebble-3.css');
	} else {
		document.getElementById('pebble-stylesheet').setAttribute('href', 'pebble-1.css');
	}
	...
	var window_layout = getQueryParam("window_layout", 0);
	...
	window_layout = parseInt(isNumber(window_layout) && window_layout <= 3 ? window_layout : 0);

	$('#font_style_'+font_style).prop( "checked", true ).checkboxradio("refresh");
	$("#idle_timeout").val(idle_timeout).selectmenu("refresh");
	$('#window_layout_'+window_layout).prop( "checked", true ).checkboxradio("refresh");
